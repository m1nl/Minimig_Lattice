...
generic (
	...
	mclk_to_lrclk : integer := 256; -- Must be an integer multiple of samplewidth
	samplerate : integer := 44100;
	samplewidth : integer := 16
);
port (
	sysclk : in std_logic;
	reset_n : in std_logic;
	--
	left_in : std_logic_vector(samplewidth-1 downto 0);
	right_in : std_logic_vector(samplewidth-1 downto 0);
	--
	mclk : out std_logic;
	lrclk : out std_logic;
	sclk : out std_logic;
...
begin
	...
	sclock : block
		constant sclk_ticks : integer := (mclk_to_lrclk / samplewidth) - 1;
	...
	lrclock : block
		signal lrclk_count : unsigned(4 downto 0) := to_unsigned(samplewidth-1,5);
		signal shift : std_logic_vector(samplewidth-1 downto 0);
	begin
		lrclk <= lrclk_out;
		sdata <= sdata_out;
		...
		process(sysclk,reset_n) begin
			if reset_n='0' then
				lrclk_count<=to_unsigned(samplewidth-1,lrclk_count'high+1);
				lrclk_out <= '0';
			elsif rising_edge(sysclk) then
